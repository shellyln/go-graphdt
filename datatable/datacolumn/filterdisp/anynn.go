...
func AnyNonNull[T any](
	...
	case Op_Fast_Eq:
		val, ok := col.Cast(v)
		if !ok {
			err = errors.New("AnyNonNull: Incorrect parameter type")
		} else {
			fn = fastfilter.XEquals(col, val, cmpEq)
		}
	case Op_Fast_NotEq:
		val, ok := col.Cast(v)
		if !ok {
			...
			fn = fastfilter.XNotEquals(col, val, cmpEq)
		}
	case Op_Fast_IsNull:
		fn = filter.False()
	...
	case Op_Fast_In:
		...
		if !ok {
			...
			fn = fastfilter.XIn(col, val, cmpEq)
		}
	case Op_Fast_NotIn:
		val, ok := col.CastArray(v)
		if !ok {
			...
			fn = fastfilter.XNotIn(col, val, cmpEq)
		}
	case Op_Fast_Like,
		Op_Fast_NotLike,
		Op_Fast_Match,
		...
		Op_Fast_Includes,
		Op_Fast_Excludes:
		err = errors.New("AnyNonNull: Invalid operator for the column type")
	default:
		err = errors.New("AnyNonNull: Unknown operator")
	}
